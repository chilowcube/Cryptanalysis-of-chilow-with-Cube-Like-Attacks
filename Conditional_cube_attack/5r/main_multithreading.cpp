...
int main( int argc, char * argv[] )
{
    ...
    vector<int> cube_t = {23, 31, 32, 34, 40, 48, 50, 55, 57};
    vector<int> invo_k = {111, 113, 115, 117, 119, 121, 123, 125, 127, 24, 30, 31, 32, 33, 35, 39, 41, 47, 49, 51, 54, 56, 58};
    

    ThreadPool thread_pool{256};
    cout << "Number of threads: " << thread_pool.num_threads() << endl;
    vector<std::future<void>> futures;



    for(uint64_t i = 0; i <= 0b11111111111111111111111; i++)
    {
	...
	for(int j = 0; j < 23; j++)
	{
    	...
    	for(uint64_t j = 0; j <= 0b11111111111111111; j++)
    	{
    	    //bitset<32> tmp(0x0);
	...
	if(flag == 0)
		cout << "under the key: " << i << "  the assumption is invalid!" << endl;
    }
