		...
		for(auto it: monomial)
		{
			if(it != "")
			{
				...
				for(auto tt: term)
				{
					...
					{
						//cout << tt << endl;
						num = num | (uint64_t(0x1)<< (stoi(tt)-64)  );
					}
				}
...
//void laststep(const unordered_map<uint64_t, Polynomial> &a, const unordered_map<uint64_t, Polynomial> &b, int bit)
//{
//	
//	fstream wd;
...
void laststep(const unordered_map<uint64_t, Polynomial> &a, const unordered_map<uint64_t, Polynomial> &b, int bit)
{
	...
	fstream wd;
	wd.open("./res/4r_"+to_string(bit) + ".txt", ios::out);
	int count_ = 0;
	int degree_min = 20;
	int degree_max = 0;
	Polynomial res;
	...
	for(auto it = a.begin(); it != a.end(); it++)
	{
		auto itt = b.find(0x55550000^(it->first));
		if(itt != b.end())
		{
			Polynomial c = it->second * itt -> second;
			res = res+c;
...
int main(int argc, char *argv[])
{
	...
	Polynomial K[128];
        ...
        vector<int> Cube_c = {16, 18, 20, 22, 24, 26, 28, 30}; 
        vector<int> Cube_t = {};

	for (int i = 0; i < 32; i++) {
	    if (find(Cube_c.begin(), Cube_c.end(), i) != Cube_c.end()) {
	        X[i] = Polynomial("t" + to_string(i+64));
	    } else {
	        X[i] = Polynomial("0");
	    }
	}
	...
	for(int i = 0; i < 15; i++)
	{
		laststep(r4[(i+1)%15], r4[(i+2)%15], i);
	}


	for(int i = 0; i < 17; i++)
	{
		laststep(r4[(i+1)%17+15], r4[(i+2)%17+15], i+15);
	}
